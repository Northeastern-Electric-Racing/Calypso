...
const STATUS_MAP: HashMap<u8, HashMap<&str, u8>> = HashMap::new();
STATUS_MAP.insert(6, HashMap::new()); // Failsafe Statuses
STATUS_MAP.insert(7, HashMap::new()); // DTC Status 1
STATUS_MAP.insert(8, HashMap::new()); // DTC Status 2
STATUS_MAP.insert(9, HashMap::new()); // Current Limits Status
STATUS_MAP.insert(12, HashMap::new()); // MPE State
STATUS_MAP.insert( // VSM State
   64,
   hashmap![
      "VSM Start State" => 0,
...
STATUS_MAP.insert( // Inverter State
   65,
   hashmap![
      "Power on State" => 0,
      "Stop State" => 1,
...
STATUS_MAP.insert(// Inverter Run Mode
   67, hashmap!["Inverter Run Mode" => 0]);
STATUS_MAP.insert(// Inverter Command Mode
   69, hashmap!["Inverter Command Mode" => 0]);
STATUS_MAP.insert(// Inverter Enable State
   70, hashmap!["Inverter Enable State" => 0]);
STATUS_MAP.insert(// Inverter Enable Lockout
   71, hashmap!["Inverter Enable Lockout" => 0]);
STATUS_MAP.insert(// Direction Command
   72, hashmap!["Direction Command" => 0]);
STATUS_MAP.insert(// BMS Active
   73, hashmap!["BMS Active" => 0]);
STATUS_MAP.insert(// BMS Limiting Torque
   74, hashmap!["BMS Limiting Torque" => 0]);
STATUS_MAP.insert(// POST Fault Lo
   75,
   hashmap![
...
STATUS_MAP.insert(// Direction Command
   84, hashmap!["Direction Command" => 0]);
STATUS_MAP.insert(// Inverter Enable
   85, hashmap!["Inverter Enable" => 0]);
STATUS_MAP.insert(// Inverter Discharge
   86, hashmap!["Inverter Discharge" => 0]);
STATUS_MAP.insert(// Speed Mode Enable
   87, hashmap!["Speed Mode Enable" => 0]);
STATUS_MAP.insert(// Cell Voltage Info
);
...
fn get_statuses(data: &Data) -> HashMap<&str, i32> {
    ...
    return bitmap.iter().map(|(name, index)| (*name, (data.value >> index) & 1)).collect();
}
