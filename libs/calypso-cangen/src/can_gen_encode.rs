...
/**
 *  Trait to generate ProcMacro2TokenStreams for encode function macro
 */
        ...
        match &self.key {
            Some(_key) => {
                let field_encoders = self
                    .fields
                    .iter_mut()
                    .map(|field| field.gen_encoder_fn())
...
/**
 *  Function to generate encoder line for NetField
 */
impl CANGenEncode for NetField {
    fn gen_encoder_fn(&mut self) -> ProcMacro2TokenStream {
        let point_encoders = self
            .points
            .iter_mut()
            .map(|point| point.gen_encoder_fn())
            .collect::<Vec<ProcMacro2TokenStream>>()
            .into_iter()
            .fold(ProcMacro2TokenStream::new(), |mut acc, ts| {
                acc.extend(ts);
 ...
 */
impl CANGenEncode for CANPoint {
    fn gen_encoder_fn(&mut self) -> ProcMacro2TokenStream {
